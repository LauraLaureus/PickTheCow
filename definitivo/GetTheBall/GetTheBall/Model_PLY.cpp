...
void Model_PLY::calculateNormal(float *coord1, float *coord2, float *coord3)
{
	...
	//return norm; // retorna un puntero a una variable local ????
}
			...
			for (int iterator = 0; iterator < this->TotalFaces; iterator++)
			{
				...
				if (buffer[0] == '3')
				{
					...
					*/
					...
					//float *norm = 
					this->calculateNormal(coord1, coord2, coord3);
...
void Model_PLY::Draw()
{
	...
	// Permite que el Pipeline de OpenGL tenga acceso al espacio de direcciones del usuario 
	glEnableClientState(GL_VERTEX_ARRAY);
	glEnableClientState(GL_NORMAL_ARRAY);

	// define donde se encuentran los vectores de coordenadas de los vertices y las normales
	glVertexPointer(3, GL_FLOAT, 0, Faces_Triangles);
	glNormalPointer(GL_FLOAT, 0, Normals);

	// Visualiza los triangulos
